hyperg_test.local <- function(term_genes, chosen_genes, background_genes) {
...
enrichment.local <- function(input_genes,
...
enrichment_analyses.local <- function(snws,
                                sig_genes_vec,
...
summarize_enrichment_results.local <- function(enrichment_res,
                                         list_active_snw_genes = FALSE) {
  ## Argument checks
  if(!is.logical(list_active_snw_genes)) {
    stop("`list_active_snw_genes` should be either TRUE or FALSE")
