visualize_terms.local <- function(result_df, input_processed = NULL,
  ...
  ############ Generate Diagrams
  if (hsa_KEGG) {
    visualize_hsa_KEGG.local(hsa_kegg_ids = result_df$ID,
                       input_processed = input_processed,
                       ...)
  } else {
    visualize_term_interactions.local(result_df = result_df,
                                pin_name_path = pin_name_path)
  }
}

visualize_term_interactions.local <- function(result_df, pin_name_path) {
  ############ Initial Steps
  ## fix naming issue
...
visualize_hsa_KEGG.local <- function(hsa_kegg_ids, input_processed, max_to_plot = NULL,
  ...
  for (i in seq_len(length(hsa_kegg_ids))) {
    ...
    pw_vis_list[[pw_id]] <- color_kegg_pathway.local(pw_id = pw_id,
...
color_kegg_pathway.local <- function(pw_id, change_vec, normalize_vals = TRUE,
  ...
  pwKGML <- tempfile()
  KGML_URL <- obtain_KEGGML_URL.local(pw_id, pwKGML, quiet)
  ...
  pw_url <- obtain_colored_url.local(pw_id = pw_id,
  ...
  dl_stat <- download_kegg_png.local(pw_url, f_path, quiet)
...
obtain_KEGGML_URL.local <- function(pw_id, pwKGML, quiet = TRUE) {
  KGML_URL <- tryCatch({
...
obtain_colored_url.local <- function(pw_id, KEGG_gene_ids, fg_cols, bg_cols) {
  pw_url <- tryCatch({
...
#'
#' @return download status
download_kegg_png.local <- function(pw_url, f_path, quiet = TRUE) {
  res <- tryCatch({
...
enrichment_chart.local <- function(result_df,
                             top_terms = 10,
...
term_gene_graph.local <- function(result_df, num_terms = 10,
...
term_gene_heatmap.local <- function(result_df, genes_df, num_terms = 10,
  ...
  parse_genes.local <- function(vec, idx)
  ...
  ### Order by row
  ordering_func.local <- function(row) {
    n <- length(row)
    pow <- 2^-(0:(n-1))
    return (row %*% pow)
  }
...
UpSet_plot.local <- function(result_df, genes_df, num_terms = 10,
                       method = "heatmap",
                       use_description = FALSE,
                       low = "green", mid = "black", high = "red",
                       ...) {
