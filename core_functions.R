...
#'
#' @inheritParams input_processing.local
#' @inheritParams fetch_gene_set.local
#' @inheritParams active_snw_search.local
#' @inheritParams enrichment_analyses.local
...
#' }
run_pathfindR.local <- function(input,
                          gene_sets = "KEGG",
                          min_gset_size = 10,
                          max_gset_size = 300,
  ...
  ############ Initial Steps
  ## Gene Sets
  gset_list <- fetch_gene_set.local(gene_sets = gene_sets,
  ...
  input_testing.local(input, p_val_threshold)

  ## Process input
  message("## Processing input. Converting gene symbols,
          if necessary (and if human gene symbols provided)")
  input_processed <- input_processing.local(input, p_val_threshold,
  ...
  if (iterations == 1) {
    ...
    ## Active Subnetwork Search
    snws <- active_snw_search.local(input_for_search = input_processed,
    ...
    enrichment_res <- enrichment_analyses.local(snws = snws,
  ...
  final_res <- summarize_enrichment_results.local(combined_res,
  ...
  final_res <- annotate_term_genes.local(result_df = final_res,
  ...
  if (visualize_enriched_terms) {
    if ((gene_sets == "KEGG")) {
      visualize_terms.local(result_df = final_res,
                                 input_processed = input_processed,
                                 hsa_KEGG = TRUE,
  ...
  if (plot_enrichment_chart) {
    ...
    graphics::plot(enrichment_chart.local(result_df = final_res))
  }
...
#'
#' @examples
#' KEGG_gset <- fetch_gene_set.local()
#' GO_MF_gset <- fetch_gene_set.local("GO-MF")
fetch_gene_set.local <- function(gene_sets = "KEGG",
                           min_gset_size = 10,
                           max_gset_size = 300,
                           custom_genes = NULL,
...
input_testing.local <- function(input, p_val_threshold = 0.05) {
...
#'                                  pin_name_path = "KEGG",
#'                                  convert2alias = FALSE)
input_processing.local <- function(input, p_val_threshold = 0.05,
...
annotate_term_genes.local <- function(result_df,
                                input_processed,
                                genes_by_term = pathfindR.data::kegg_genes) {
  ### Argument checks
